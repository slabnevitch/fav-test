$(function() {
	...
	$(document).ready(function() {
		...
		// magnific popup
				var $toSubscribePopup = $('.tooltip__header-phone, .button-popup, .popup-catalog, .popup-quality, .call-order, .statistic-button--calc').magnificPopup({
					type: 'inline',
					preloader: false,
					focus: '#name',
		...
		$('.mouse-out-popup__form input').focus(function() {
		...
		// $('.to-subscribe-form, .to-validate-form, .to-manual-form, .subscribe-block__form').submit(function() {
		// 		var $th = $(this);
		// 		var newHref = 'subscribe.html';

		// 		if($th.hasClass('to-manual-form') || $th.hasClass('subscribe-block__form')){
		// 			newHref = 'manual.html';
		// 		}
	...
	// statistic fixed
		...
		if($statBlock.length > 0){
			...
			if($('.assortment-content').length > 0){
				...
				deadline = $('.free-part').offset().top - 350;
			}
	...
	// kviz
			function Kviz(){
					...
					this.kvizSubmit = function(e) {
						...
						$.ajax({
							type: "POST",
							url: "/wp-content/themes/mlv/mail.php", //Change
							data: th.serialize()
						}).done(function() {
